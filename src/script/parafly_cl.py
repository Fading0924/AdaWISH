...
parser.add_argument('-t', '--timeout', type=int, help="Timeout for each optimization instance (seconds)", default=14400)
...
for i in range(0,depth+1):			## main for loop
    parafileName = "%s_level%s_script.txt" % (fname, str(i))
    rerunfile = "%s/%s" % (outputdir, "level"+str(i)+"rerun.txt")
    ...
    for t in range(1,sampnum+1):			## main for loop
        ...
        if not os.path.exists(targetfile):
            ...
            # sys.exit(0)
            lineCnt += 1
            cmdline = ("timeout %d ./ilog -timelimit %d -paritylevel 1 -number %d %s > %s && cp %s %s/output/%s") % (args.timeout , 900, i , fname_extended , outfilenamelog, outfilenamelog, "/home/wang4588/adawish", fname+'/')
            f.write(cmdline+'\n')
        ## Parallel execution:
        ##
    ...
    f.write("cd $TMPDIR\n")
    f.write("ParaFly -c %s_level%s_script.txt -CPU %s -failed_cmds %s_level%srerun.txt\n" % (fname, str(i), str(core_num),fname, str(i)))
    f.write("cp %s_level%srerun.txt %s/" % (fname, str(i), outputdir))

    os.system("qsub -q %s -l walltime=04:00:00,nodes=1:ppn=%d,naccesspolicy=singleuser %s" % (args.queue, core_num, sname))